...
//        chatMessage("[addToBaseStat] id: %s", Integer.toString(si.getSkillId()));
    ...
    public void deductMp(int mpCost) {
        // Deduct the specified amount of MP
        int currentMp = getMP();
        int newMp = currentMp - mpCost;

        // Make sure MP doesn't go below zero
        newMp = Math.max(newMp, 0);

        // Update the character's MP
        setStat(Stat.mp, newMp);  // Using setStat to update the MP
    }
    /**
...
//                chatMessage("[addBaseStat else] key: %s value: %s", bs.toString(), Integer.toString(amount));
    ...
    public boolean applyMpCon(int skillID, int slv)
    {
        ...
        if (hasEnough// Kinesis Doesn't use MP
            ...
            System.out.println("applyMpCon called for skillID: " + skillID + ", Skill Level: " + slv);
            System.out.println("MP Consumption for skillID: " + skillID + " is: " + mpCon);
